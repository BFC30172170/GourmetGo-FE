...
test('Renders title', () => {
  setup();
  expect(screen.getByRole('heading', {level: 1})).toHaveTextContent('Restaurants near')
});

test('Renders at least one restaurant tiles', async () => {
  setup();
  const products = await screen.findAllByRole('heading', { level: 3 });
  expect(products).toHaveLength(2);
})

test('Search returns all restaurants with term in their name', async () => {
  setup();
  fireEvent.change(screen.getByPlaceholderText('Search for a Restaurant'), {target: {value: 'Ziemann Inc'}});
  await waitFor(async ()=>{
    expect(await screen.findAllByRole('heading', {level: 3})).toHaveLength(1);
    expect(screen.getByPlaceholderText('Search for a Restaurant')).toHaveValue('Ziemann Inc');
  })
})
